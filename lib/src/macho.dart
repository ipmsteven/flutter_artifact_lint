...
class MachOLinkeditData {
  const MachOLinkeditData({
  ...
  String get commandName => switch (command) {
    _lcSegmentSplitInfo => 'LC_SEGMENT_SPLIT_INFO',
    _lcDylibCodeSignDrs => 'LC_DYLIB_CODE_SIGN_DRS',
    _lcLinkerOptimizationHint => 'LC_LINKER_OPTIMIZATION_HINT',
    _lcAtomInfo => 'LC_ATOM_INFO',
    _lcFunctionVariants => 'LC_FUNCTION_VARIANTS',
...
class MachOParser {
  ...
  MachOReport _parseThinFileAt(
    ...
    final functionStarts = _readFunctionStartsFromFile(
      raf,
      fileOffset,
      availableLength,
  ...
  MachOReport _parseThin(List<int> bytes) {
    ...
    for (var i = 0; i < header.commandCount; i += 1) {
      ...
      if (command == _lcDyldEnvironment && commandSize >= 12) {
        ...
        if (value != null) {
          dyldEnvironments.add(MachODyldEnvironment(value: value));
        }
      }
      ...
      if (_isGenericLinkeditDataCommand(command) && commandSize >= 16) {
        linkeditData.add(
          MachOLinkeditData(
            command: command,
            dataOffset: _readU32(bytes, offset + 8),
...
MachOReport _deduplicatedReport(
  ...
  final byLinkeditData = <String, MachOLinkeditData>{};
...
bool _isGenericLinkeditDataCommand(int command) {
  return {
    _lcSegmentSplitInfo,
    _lcDylibCodeSignDrs,
    _lcLinkerOptimizationHint,
    _lcAtomInfo,
    _lcFunctionVariants,
...
List<MachOFunctionStarts> _readFunctionStartsFromFile(
  RandomAccessFile raf,
  int fileOffset,
  int availableLength, {
  required List<MachOFunctionStarts> functionStarts,
}) {
  final result = <MachOFunctionStarts>[];
  for (final functionStart in functionStarts) {
    if (!_canReadDyldInfoStream(
      functionStart.dataOffset,
      functionStart.dataSize,
      availableLength,
    )) {
      result.add(functionStart);
      continue;
    }

    result.add(
      MachOFunctionStarts(
        dataOffset: functionStart.dataOffset,
        dataSize: functionStart.dataSize,
        offsets: _parseFunctionStartOffsets(
          _readRange(
            raf,
            fileOffset + functionStart.dataOffset,
            functionStart.dataSize,
          ),
        ),
      ),
...
List<MachOFunctionStarts> _readFunctionStartsFromBytes(
  List<int> bytes, {
  required List<MachOFunctionStarts> functionStarts,
}) {
  final result = <MachOFunctionStarts>[];
    ...
    result.add(
      MachOFunctionStarts(
        ...
        offsets: _parseFunctionStartOffsets(
          bytes.sublist(
            functionStart.dataOffset,
            functionStart.dataOffset + functionStart.dataSize,
          ),
        ),
      ),
    );
...
List<int> _parseFunctionStartOffsets(List<int> bytes) {
  ...
  var cursor = 0;
  var currentOffset = 0;
  while (cursor < bytes.length && offsets.length < _maxFunctionStartCount) {
    final result = _readUleb128(bytes, cursor);
    if (result == null) break;
    cursor = result.nextOffset;
    if (result.value == 0) break;

    currentOffset += result.value;
...
const _lcSegmentSplitInfo = 0x1e;
